...
void VisionEngine::saveBuffer(unsigned char* buffer, int bytes) {
    ...
    if (stat(".\\recording",&info)!=0) {
		...
		LPWSTR buf;
		MultiByteToWideChar(CP_ACP, 0, dir.c_str(), -1, buf, MAX_PATH);
        CreateDirectory(buf,attr);
    }
...
void VisionEngine::mainLoop()
{
    ...
    while(running_) {
		...
		unsigned char image[] = {
			...
			0, 255, 0, 0,255,0,1,2,3,4,5,6,6,7,8,9,45,27,98,26,84,200,100  // 2nd RGBA pixel, completely blue.
			// and so on
		};
		cameraReadBuffer = image;
		std::cout << cameraReadBuffer;
